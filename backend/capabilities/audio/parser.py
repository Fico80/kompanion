...
_PERCENT_RE = rf"\b({NUMBER_PATTERN})\s*(?:prozent|%)"
...
def parse_app_volume(text_lower: str) -> dict | None:
    ...
    vol_keyword = re.search(
        rf"\b({NUMBER_PATTERN})\s*(?:prozent|%)"
        rf"|\b(lautstärke|volume)\s+(?:auf\s+|to\s+)?({NUMBER_PATTERN})\b"
    ...
    for keyword, info in sorted(apps.items(), key=lambda x: len(x[0]), reverse=True):
        if keyword not in text_lower:
            continue
        m_pct = re.search(rf"\b({NUMBER_PATTERN})\s*(?:prozent|%)|\b(?:lautstärke|volume)\s+(?:auf\s+|to\s+)?({NUMBER_PATTERN})\b", text_lower)
        if m_pct:
            val = str(parse_number(m_pct.group(1) or m_pct.group(2)))
            volume = f"{val}%"
...
def parse_audio(text_lower: str) -> dict | None:
    ...
    if re.search(r"\bton an\b|\b(unmute|entstummen?|laut\s+machen|laut\s+stellen|sound\s+on|turn\s+on\s+sound|enable\s+sound)\b", text_lower):
    ...
    m = re.search(rf"\b(lautstärke|volume|lautheit)\s+(?:auf\s+|to\s+|set\s+to\s+)?({NUMBER_PATTERN})\s*(?:prozent|%)?\b", text_lower)
    if m:
        val = min(parse_number(m.group(2)), 150)
    ...
    if re.search(r"\b(lauter|louder|volume up|hochdrehen|lauter\s+stellen|turn\s+up|raise\s+volume|increase\s+volume)\b", text_lower):
        step = re.search(_PERCENT_RE, text_lower)
        n = str(parse_number(step.group(1))) if step else "10"
    ...
    has_music_ctx = re.search(r"\b(musik|song|lied|track|abspielen|wiedergabe|music|playback|audio)\b", text_lower)
    plain_resume = re.search(r"\bresume\b", text_lower) and not has_music_ctx
    if not plain_resume and re.search(r"\b(play|weiterspielen|fortsetzen|abspielen|spielen|resume|continue\s+playing)\b|\bspiele?\b.{0,25}\b(ab|musik|lied|song|track)\b|\bmusik\s+(an|start|abspielen|spielen|ab)\b|\bstarte\s+(musik|lied|song|track)\b|\bmach\s+(die\s+)?musik\s+an\b", text_lower):
